...
function toISODateInTZ(d: Date, timeZone: string) {
...
function yesterdayISO_ET(): string {
  const now = new Date();
  const y = new Date(now);
  y.setDate(y.getDate() - 1);
  return toISODateInTZ(y, TZ);
}
...
serve(async (req) => {
  ...
  try {
    ...
    const cutoffEndDate = yesterdayISO_ET(); // if end_date <= yesterday, tournament is definitely over

    // ✅ Uses end_date (DATE), not end_iso
    const { data: tournaments, error: tErr } = await supabase
      .from("tournaments")
      .select("id, status, payout_run_at, entry_fee_cents, end_date")
      .lte("end_date", cutoffEndDate)
      .eq("status", "resolved")
    ...
    for (const t of tournaments as any[]) {
      ...
      if (upErr) console.error("tournament update error", tournamentId, upErr);
    ...
    return new Response(JSON.stringify({ ok: true, cutoffEndDate, payoutSummary }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (err: any) {
