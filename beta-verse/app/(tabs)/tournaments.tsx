// app/(tabs)/Dash.js — SportsDataIO feed with trial-key fix (?key=...), original colorway, centered pill + locked alignment
// Make sure your key is available as EXPO_PUBLIC_SPORTSDATAIO_KEY (or in app.json under expo.extra.SPORTSDATAIO_KEY)
...
const withKey = (url) =>
  SDIO_KEY ? `${url}${url.includes("?") ? "&" : "?"}key=${encodeURIComponent(SDIO_KEY)}` : url;
...
export default function Dash(){
  ...
  useEffect(()=>{ if(!SDIO_KEY)return; let off=false; (async()=>{
      try{
        const r=await fetch(withKey(`${sportCfg.base}/${sportCfg.teams}`),{headers});
        if(!r.ok){ const t=await r.text().catch(()=>"-"); throw new Error(`Teams ${sportCfg.label} -> ${r.status} ${t}`); }
  ...
  useEffect(()=>{ if(!SDIO_KEY)return; let off=false; (async()=>{
      try{
        ...
        const r=await fetch(withKey(`${sportCfg.base}/${sportCfg.standings(season)}`),{headers});
        if(!r.ok){ console.warn("Standings HTTP",sportCfg.label,r.status, await r.text().catch(()=>"-")); if(!off) setStandingsMap({}); return; }
  ...
  async function fetchByDate(date){
    ...
    const url=withKey(`${sportCfg.base}/${sportCfg.gamesByDate}/${encodeURIComponent(d)}`);
    const r=await fetch(url,{headers});
    if(!r.ok){ const msg=await r.text().catch(()=>"-"); console.warn(`GamesByDate ${sportCfg.label} ${r.status}`, msg); return []; }
